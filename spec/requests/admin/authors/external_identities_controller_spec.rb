...
RSpec.describe Admin::Authors::ExternalIdentitiesController do
  let(:author) { create(:author) }
  let(:external_identity) do
    create(:external_identity, owner: author, external_resource: :open_library, external_id: 'OL1A')
  end
  ...
  describe 'GET /admin/authors/:author_id/external_identities/:id' do
    let(:send_request) do
      get admin_author_external_identity_path(author, external_identity), headers: authorization_header
    end

    it 'renders a successful response with identity details' do
      send_request
      expect(response).to be_successful
      expect(response).to render_template 'admin/authors/external_identities/show'
      expect(response.body).to include('Open Library')
      expect(response.body).to include('OL1A')
    end
  end

  describe 'GET /admin/authors/:author_id/external_identities/new' do
    let(:send_request) { get new_admin_author_external_identity_path(author), headers: authorization_header }

    it 'renders a successful response' do
      send_request
      expect(response).to be_successful
      expect(response).to render_template 'admin/authors/external_identities/new'
    end
  end

  describe 'GET /admin/authors/:author_id/external_identities/:id/edit' do
    let(:send_request) do
      get edit_admin_author_external_identity_path(author, external_identity), headers: authorization_header
    end

    it 'renders a successful response' do
      send_request
      expect(response).to be_successful
      expect(response).to render_template 'admin/authors/external_identities/edit'
    end
  end

  describe 'POST /admin/authors/:author_id/external_identities' do
    context 'with valid parameters' do
      let(:send_request) do
        post admin_author_external_identities_path(author),
  ...
  describe 'PATCH /admin/authors/:author_id/external_identities/:id' do
    context 'with valid parameters' do
      ...
      let(:send_request) do
        patch admin_author_external_identity_path(author, external_identity),
      ...
      it 'redirects to the external identity' do
        send_request
        expect(response).to redirect_to(admin_author_external_identity_path(author, external_identity))
    ...
    context 'with invalid parameters' do
      let!(:other_identity) do
        create(:external_identity, owner: author, external_resource: :wikidata, external_id: 'Q999')
      end
      let(:send_request) do
        patch admin_author_external_identity_path(author, external_identity),
              params: {
  ...
  describe 'DELETE /admin/authors/:author_id/external_identities/:id' do
    let(:send_request) do
      delete admin_author_external_identity_path(author, external_identity), headers: authorization_header
    end
