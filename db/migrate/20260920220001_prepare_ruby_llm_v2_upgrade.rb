...
class PrepareRubyLlmV2Upgrade < ActiveRecord::Migration[8.1]
  ...
  def validate_upgrade
    %i[ai_chats ai_messages].each do |table|
  ...
  def validate_legacy_message_columns
    ...
    return if legacy_columns.any? { |column| column_exists?(:ai_messages, column) }
    return unless table_exists?(:ruby_llm_usages)

    raise 'ai_messages already has the RubyLLM 2.0 schema. ' \
  ...
  def validate_usage_identities(models)
    ...
    raise "Could not determine provider and model for ai_messages id #{missing}. " \
          'Set its model reference, or its chat model reference, before retrying.'
  end

  def validate_unique_tool_results
    column = :tool_call_id
    return unless column_exists?(:ai_messages, column)

    duplicates = migration_record(:ai_messages)
  ...
  def add_chat_and_message_columns
    ensure_boolean_column(:ai_chats, :cancelled)
    ensure_boolean_column(:ai_messages, :cache_until_here)
    with_upgrade_safety do
      change_table :ai_messages, bulk: true do |table|
        table.string :finish_reason unless column_exists?(:ai_messages, :finish_reason)
        table.json :citations unless column_exists?(:ai_messages, :citations)
        table.json :server_tool_calls unless column_exists?(:ai_messages, :server_tool_calls)
        table.json :raw_content unless column_exists?(:ai_messages, :raw_content)
        table.json :raw_reasoning unless column_exists?(:ai_messages, :raw_reasoning)
      end
    end
  end
  ...
  def move_tool_calls
    ...
    with_upgrade_safety do
      change_table table, bulk: true do |definition|
        ...
        definition.column :result_id, reference_type_for(:ai_messages) unless column_exists?(table, :result_id)
    ...
    unless table_exists?(:ruby_llm_usages)
      create_table :ruby_llm_usages, id: :bigint do |table|
        table.references :chat, polymorphic: true, null: false,
                                type: reference_type_for(:ai_chats), index: false
        table.references :message, polymorphic: true,
                                   type: reference_type_for(:ai_messages), index: false
  ...
  def create_batches
    unless table_exists?(:ruby_llm_batches)
      create_table :ruby_llm_batches, id: :bigint do |table|
        ...
        table.json :request_counts
        table.json :reported_cost
        table.timestamps
      end
    end
  ...
  def usage_identity_sql(models)
    ...
    model_ids = []
    joins = []
    if column_exists?(:ai_messages, :model_id)
      model_column = column_definition(:ai_messages, :model_id)
      if model_column.type == :string
        providers << message_value(:provider) if column_exists?(:ai_messages, :provider)
    ...
    chat_model_column = first_existing_column(:ai_chats, :model_id, :ruby_llm_model_id)
    if chat_model_column
      joins << "LEFT JOIN #{quote_table(:ai_chats)} legacy_chats " \
               "ON #{message_value(:chat_id)} = legacy_chats.#{quoted_primary_key(:ai_chats)}"
  ...
  def legacy_usage_conditions
    ...
    conditions = columns.filter_map do |column|
      "#{message_value(column)} IS NOT NULL" if column_exists?(:ai_messages, column)
    end
    conditions << "#{message_value(:role)} = 'assistant'" if column_exists?(:ai_messages, :role)
    conditions
  end
  ...
  def normalize_json_defaults(table, defaults)
    defaults.each do |column_name, value|
      ...
      # SQLite cannot quote Hash/Array column defaults; persist JSON text instead.
      serialized = value.is_a?(String) ? value : value.to_json
      with_upgrade_safety { change_column_default table, column_name, from: nil, to: serialized }
    end
  end
