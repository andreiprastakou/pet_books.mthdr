...
class BackfillRubyLlmV2Data < ActiveRecord::Migration[8.1]
  ...
  def verify_upgrade_in_progress
    ...
    unless legacy_columns.any? { |column| column_exists?(:ai_messages, column) }
  ...
  def backfill_message_content
    return mark_completed('message_content') unless column_exists?(:ai_messages, :content_raw)
  ...
  def backfill_tool_results
    ...
    return mark_completed('tool_results') unless column_exists?(:ai_messages, result_reference)

    each_message_range('tool_results') do |range|
      messages = quote_table(:ai_messages)
      tool_calls = quote_table(:ruby_llm_tool_calls)
      message_id = quoted_primary_key(:ai_messages)
        ...
        UPDATE #{tool_calls}
           SET result_id = (
                 SELECT legacy_messages.#{message_id}
                  ...
                  WHERE #{range}
                    AND #{result_id} = #{tool_calls}.#{tool_call_id}
                  ORDER BY legacy_messages.#{message_id}
                  LIMIT 1
               ),
  ...
  def each_message_range(task)
    ...
    primary_key = connection.primary_key(:ai_messages)
    records = migration_record(:ai_messages)
  ...
  def verify_message_content
    return unless column_exists?(:ai_messages, :content_raw)

    missing = migration_record(:ai_messages)
              .where.not(content_raw: nil)
              .where('raw_content IS NULL OR content IS NULL')
              .limit(1)
              .pick(connection.primary_key(:ai_messages))
    ...
    missing = select_value(<<~SQL)
      SELECT legacy_messages.#{quoted_primary_key(:ai_messages)}
        ...
        LEFT JOIN #{tool_calls} migrated_tool_calls
         ...
         AND migrated_tool_calls.result_type = #{connection.quote('Admin::Ai::Message')}
         AND migrated_tool_calls.result_id = legacy_messages.#{quoted_primary_key(:ai_messages)}
  ...
  def usage_identity_sql
    ...
    if column_exists?(:ai_messages, :model_id)
      explicit_model = column_definition(:ai_messages, :model_id)
    ...
    if column_exists?(:ai_chats, :ruby_llm_model_id)
      chats = quote_table(:ai_chats)
  ...
  def legacy_usage_conditions
    ...
    if column_exists?(:ai_messages, :cost_details)
      ...
      WHERE (#{conditions.join(' OR ')})
        ...
        AND NOT EXISTS (
          SELECT 1
           ...
           WHERE existing_usages.message_type = #{connection.quote('Admin::Ai::Message')}
             AND (#{usage_range})
             AND existing_usages.message_id = #{message_value(connection.primary_key(:ai_messages))}
  ...
  def usage_insert_sql(candidate, provider, model)
    ...
    values = [
      connection.quote('Admin::Ai::Chat'),
      message_value(:chat_id),
      connection.quote('Admin::Ai::Message'),
      message_value(connection.primary_key(:ai_messages)),
  ...
  def preferred_message_value(preferred, fallback)
    return message_value(preferred) if column_exists?(:ai_messages, preferred)

    legacy_message_value(fallback)
  end

  def legacy_message_value(column, fallback: 'NULL')
    return fallback unless column_exists?(:ai_messages, column)

    message_value(column)
  end

  def legacy_cost_detail(key)
    return 'NULL' unless column_exists?(:ai_messages, :cost_details)
  ...
  def create_progress_table
    ...
    create_table PROGRESS_TABLE, id: false do |table|
      ...
      table.column :last_id, reference_type_for(:ai_messages)
