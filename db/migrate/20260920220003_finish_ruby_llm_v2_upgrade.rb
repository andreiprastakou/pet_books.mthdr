...
class FinishRubyLlmV2Upgrade < ActiveRecord::Migration[8.1]
  ...
  def relax_legacy_message_constraints
    %i[model_id tool_call_id].each do |column|
      next unless column_definition(:ai_messages, column)&.null == false

      with_upgrade_safety { change_column_null :ai_messages, column, true }
    end
  end
  ...
  def enforce_required_defaults
    ...
    enforce_not_null(:ai_chats, :cancelled)
    enforce_not_null(:ai_messages, :cache_until_here)
       ...
       WHERE (#{conditions.join(' OR ')})
         AND NOT EXISTS (
           SELECT 1
            ...
            WHERE migrated_usages.message_type = #{connection.quote('Admin::Ai::Message')}
              AND migrated_usages.message_id = #{message_value(connection.primary_key(:ai_messages))}
  ...
  def usage_identity_joins
    ...
    if column_exists?(:ai_messages, :model_id) &&
       column_definition(:ai_messages, :model_id).type != :string
