module Admin
  module Authors
    class WikidataWorksFetchesController < AdminController
      before_action :fetch_author
      before_action :fetch_external_identity

      def create
        task = Admin::Tasks::WikidataAuthorWorksFetch.setup(@external_identity)
        task.enqueue_for_processing!
        redirect_to admin_author_path(@author), notice: t('notices.admin.wikidata_works_fetches.create.success')
      end
