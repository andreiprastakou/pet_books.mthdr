...
RSpec.describe Admin::Tasks::WikidataAuthorSearch do
  describe 'validation' do
    it 'has a valid factory' do
      expect(build(:wikidata_author_search_task)).to be_valid
    end
  end
  ...
  describe '.setup' do
    ...
    it 'creates a new wikidata author search task' do
  ...
  describe '#perform' do
    ...
    let(:results) do
      [
        ...
        { 'id' => 'Q81738', 'display-label' => { 'language' => 'en', 'value' => "Tolkien's legendarium" } }
      ]
    end
  ...
  describe '.next_unresolved' do
    let!(:first_task) { create(:wikidata_author_search_task, status: :fetched) }
    let!(:second_task) { create(:wikidata_author_search_task, status: :fetched) }

    before { create(:wikidata_author_search_task, status: :verified) }
