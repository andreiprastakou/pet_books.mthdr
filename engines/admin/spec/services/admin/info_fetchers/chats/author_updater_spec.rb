...
RSpec.describe Admin::InfoFetchers::Chats::AuthorUpdater do
  ...
  describe '#apply_updates' do
    ...
    let(:ai_talker) { instance_double(Admin::InfoFetchers::Chats::AuthorsExpert) }
    let(:author_info) do
      {
    ...
    before do
      allow(Admin::InfoFetchers::Chats::AuthorsExpert).to receive(:new).and_return(ai_talker)
